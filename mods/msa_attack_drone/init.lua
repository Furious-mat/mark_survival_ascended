...
local path = minetest.get_modpath("msa_attack_drone")
...
function msa_attack_drone.hq_linear_pursuit(self, prty, target)
	local move_resist = 0
	...
	local func = function(self)
		...
		elseif dist > 3 then
			...
			if pos.y - tpos.y > 7 then
				msa_attack_drone.hq_swoop_to_target(self, prty + 1, target)
				return
			end
...
function msa_attack_drone.hq_swoop_to_target(self, prty, target)
	local move_resist = 0
	local stage = 1
	local tpos = {}
	local func = function(self)
...
function msa_attack_drone.hq_roam(self, prty)
	local move_resist = 0
	local idle_timer = 2
	local pos2 = nil
	local init = false
...
local function attack_drone_logic(self)
    ...
    if mobkit.timer(self, 1) then
		...
		if prty < 2
		and player then
			msa_attack_drone.hq_linear_pursuit(self, 2, player)
		end

        if mobkit.is_queue_empty_high(self) then
            msa_attack_drone.hq_roam(self, 0)
        end
    end
end

minetest.register_entity("msa_attack_drone:gamma_attack_drone",{
	-- Initial
	physical = false,
    ...
    on_punch = function(self, puncher, _, tool_capabilities, dir)
		...
		if mobkit.is_alive(self) then
			...
			if mobkit.is_queue_empty_high(self) then
				msa_attack_drone.hq_linear_pursuit(self, 20, puncher)
			end
		end
	end
})

mob_core.register_spawn_egg("msa_attack_drone:gamma_attack_drone", "89db52", "66a43d")

minetest.register_entity("msa_attack_drone:beta_attack_drone",{
	-- Initial
	physical = false,
	collide_with_objects = false,
	visual = "mesh",
...
mob_core.register_spawn_egg("msa_attack_drone:beta_attack_drone", "a3bcd1cc", "527fa3d9")

minetest.register_entity("msa_attack_drone:alpha_attack_drone",{
	-- Initial
	physical = false,
	collide_with_objects = false,
	visual = "mesh",
...
mob_core.register_spawn_egg("msa_attack_drone:alpha_attack_drone", "74271acc", "250b06d9")

minetest.register_entity("msa_attack_drone:pteranodon",{
	-- Initial
	physical = true,
...
mob_core.register_spawn_egg("msa_attack_drone:pteranodon", "90431bcc", "16120dd9")
