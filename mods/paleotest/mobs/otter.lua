...
local otter_inv_size = 2 * 5
local inv_otter = {}
inv_otter.otter_number = tonumber(storage:get("otter_number") or 1)
...
local function give_xp_to_nearby_players(pos)
    ...
    for _, obj in ipairs(players) do
        if obj:is_player() then
            xp_redo.add_xp(obj:get_player_name(), 1)
        end
    end
end

local function otter_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:otter_inv")
            end
        end
        
        minetest.remove_detached_inventory("otter_" .. self.otter_number)
        ...
        if self.status ~= "sleeping" then
            ...
            if prty < 14 and self.hunger < self.max_hunger then
                if self.feeder_timer == 1 then
                    paleotest.hq_go_to_feeder(self, 14, "paleotest:feeder_piscivore")
                end
            end
            ...
            if prty < 4 then
                if math.random(1, self.hunger) == 1 then
                    paleotest.hq_graze(self, 4, paleotest.global_prime_fish)
                    return
                end
            end
        end
...
minetest.register_entity("paleotest:otter", {
    -- Stats
    max_hp = 40,
    ...
    visual = "mesh",
    mesh = "paleotest_otter.b3d",
    female_textures = {"paleotest_otter.png"},
    male_textures = {"paleotest_otter.png"},
    child_textures = {"paleotest_otter.png"},
    animation = {
    ...
    sounds = {
        ...
        random = {
            name = "paleotest_otter1",
            gain = 1.0,
            distance = 16
        },
        hurt = {
            name = "paleotest_otter2",
            gain = 1.0,
            distance = 16
        },
        death = {
            name = "paleotest_otter3",
            gain = 1.0,
            distance = 16
        }
    },
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.otter_number = inv_otter.otter_number
    inv_otter.otter_number = inv_otter.otter_number + 1
    storage:set_int("otter_number", inv_otter.otter_number)
    local inv = minetest.create_detached_inventory("paleotest:otter_" .. self.otter_number, {})
    inv:set_size("main", otter_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:field_guide" then
            ...
            minetest.show_formspec(clicker:get_player_name(),
                                   "paleotest:otter_guide",
                                   paleotest.register_fg_entry(self, {
                female_image = "paleotest_otter_fg.png",
                male_image = "paleotest_otter_fg.png",
                diet = "Piscivore",
                temper = "Friendly"
            }))
        end
        if clicker:get_wielded_item():get_name() == "" and clicker:get_player_control().sneak == false and clicker:get_player_name() == self.owner then
        minetest.show_formspec(clicker:get_player_name(), "paleotest:otter_inv",
            "size[8,9]" ..
            "list[detached:paleotest:otter_" .. self.otter_number .. ";main;0,0;5,2;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:otter_" .. self.otter_number .. ";main]" ..
...
mob_core.register_spawn_egg("paleotest:otter", "6e512dcc", "2f1702d9")

minetest.register_craftitem("paleotest:otter_dossier", {
	description = "Otter Dossier",
	stack_max= 1,
	inventory_image = "paleotest_otter_fg.png",
	groups = {dossier = 1},
