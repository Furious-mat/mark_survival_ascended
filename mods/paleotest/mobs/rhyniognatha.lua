...
local function rhyniognatha_logic(self)
...
minetest.register_entity("paleotest:rhyniognatha", {
    -- Stats
    max_hp = 900,
    ...
    -- Visual
    collisionbox = {-0.9, 0, -0.9, 0.9, 1.8, 0.9},
    visual_size = {x = 25, y = 25},
    scale_stage1 = 0.25,
    scale_stage2 = 0.5,
    scale_stage3 = 0.75,
    visual = "mesh",
    mesh = "paleotest_rhyniognatha.b3d",
    female_textures = {"paleotest_rhyniognatha.png"},
    male_textures = {"paleotest_rhyniognatha.png"},
    child_textures = {"paleotest_rhyniognatha.png"},
    animation = {
    ...
    drops = {
        ...
        {name = "paleotest:rhyniognatha_pheromone", chance = 3, min = 1, max = 1}
    },
    timeout = 0,
    logic = rhyniognatha_logic,
    ...
    on_rightclick = function(self, clicker)
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:field_guide" then
            minetest.show_formspec(clicker:get_player_name(),
                                   "paleotest:rhyniognatha_guide",
                                   paleotest.register_fg_entry(self, {
                female_image = "paleotest_rhyniognatha_fg.png",
                male_image = "paleotest_rhyniognatha_fg.png",
                diet = "Omnivore",
                temper = "Aggressive"
            }))
        end
...
mob_core.register_spawn_egg("paleotest:rhyniognatha", "aca483cc", "231d14d9")

----------
-- Rhyniognatha Pheromone --
----------

minetest.register_craftitem("paleotest:rhyniognatha_pheromone", {
    description = "Rhyniognatha Pheromone",
    stack_max= 100,
    inventory_image = "paleotest_rhyniognatha_pheromone.png",
    on_use = function(itemstack, user, pointed_thing)
        local pos = user:get_pos()
        minetest.add_entity(pos, "paleotest:Rhyniognatha")
        itemstack:take_item()
        return itemstack
    end,
})
...
minetest.register_craftitem("paleotest:resin", {
	description = "Resin",
	stack_max= 100,
	inventory_image = "paleotest_resin.png",
})
...
minetest.register_node("paleotest:resin_block", {
    description = "Resin Block",
    tiles = {"paleotest_resin_block.png"},
    paramtype2 = "facedir",
...
minetest.register_craftitem("paleotest:rhyniognatha_dossier", {
	description = "Rhyniognatha Dossier",
	stack_max= 1,
	inventory_image = "paleotest_rhyniognatha_fg.png",
	groups = {dossier = 1},
	on_use = function(itemstack, user, pointed_thing)
...
---------------------
-- Rhyniognatha (Tamable) --
--------------------

local modname = minetest.get_current_modname()
local storage = minetest.get_mod_storage()

local rhyniognatha_inv_size = 20 * 8
local inv_rhyniognatha = {}
inv_rhyniognatha.rhyniognatha_number = tonumber(storage:get("rhyniognatha_number") or 1)
...
local function deserialize_inventory(inv, data)
    local items = data
    for i = 0, rhyniognatha_inv_size do
...
local function give_xp_to_nearby_players(pos)
    ...
    for _, obj in ipairs(players) do
        if obj:is_player() then
            xp_redo.add_xp(obj:get_player_name(), 1000)
        end
    end
end
...
local function rhyniognatha_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:rhyniognatha_inv")
            end
        end
        
        minetest.remove_detached_inventory("rhyniognatha_" .. self.rhyniognatha_number)
        mob_core.on_die(self)
        give_xp_to_nearby_players(pos)
        return
    end
        ...
        if self.isonground then
            ...
            if prty < 10 then
                if player and not self.child then
                    ...
                    if self.mood > 0 and player:get_player_name() ~= self.owner then
                        mob_core.logic_attack_player(self, 10, player)
...
minetest.register_entity("paleotest:Rhyniognatha", {
    ...
    driver_eye_offset = {{x = 0, y = 35, z = 5}, {x = 0, y = 45, z = 55}},
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.rhyniognatha_number = inv_rhyniognatha.rhyniognatha_number
    inv_rhyniognatha.rhyniognatha_number = inv_rhyniognatha.rhyniognatha_number + 1
    storage:set_int("rhyniognatha_number", inv_rhyniognatha.rhyniognatha_number)
    local inv = minetest.create_detached_inventory("paleotest:rhyniognatha_" .. self.rhyniognatha_number, {})
    inv:set_size("main", rhyniognatha_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        if self.hp <= 100 then
        paleotest.feed_tame(self, clicker, 1, true, true)
        end
        if clicker:get_wielded_item():get_name() == "paleotest:field_guide" then
            minetest.show_formspec(clicker:get_player_name(),
                                   "paleotest:rhyniognatha_guide",
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:rhyniognatha_saddle" and clicker:get_player_name() == self.owner then
            mob_core.mount(self, clicker)
        end
        if clicker:get_wielded_item():get_name() == "msa_cryopod:cryopod" then
        msa_cryopod.capture_with_cryopod(self, clicker)
        end
        if clicker:get_wielded_item():get_name() == "paleotest:sap" and self.tamed then
		mob_core.random_loot_drop(self, 1, 1, "paleotest:resin")
        end
        if clicker:get_wielded_item():get_name() == "" and clicker:get_player_control().sneak == false and clicker:get_player_name() == self.owner then
        minetest.show_formspec(clicker:get_player_name(), "paleotest:rhyniognatha_inv",
            "size[9,30]" ..
            "list[detached:paleotest:rhyniognatha_" .. self.rhyniognatha_number .. ";main;0,0;8,20;]" ..
            "list[current_player;main;0,20;9,4;]" ..
            "listring[detached:paleotest:rhyniognatha_" .. self.rhyniognatha_number .. ";main]" ..
            "listring[current_player;main]")
        end
        if self.mood > 0 then paleotest.set_order(self, clicker) end
        mob_core.protect(self, clicker, true)
        mob_core.nametag(self, clicker)
    end,
    on_punch = function(self, puncher, _, tool_capabilities, dir)
        if puncher:get_player_control().sneak == true then
            ...
            paleotest.on_punch(self)
            mob_core.on_punch_basic(self, puncher, tool_capabilities, dir)
            if puncher:get_player_name() == self.owner and self.mood > 0 then
                return
            end
            if self.hp > self.max_hp/2 then
                mob_core.logic_attack_player(self, 20, puncher)
...
minetest.register_craftitem("paleotest:feed", {
	description = "Rhyniognatha Feed",
	stack_max= 1,
	inventory_image = "paleotest_rhyniognatha_feed.png",
})
