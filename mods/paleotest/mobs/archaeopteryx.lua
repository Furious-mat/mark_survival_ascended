...
local archaeopteryx_inv_size = 1 * 8
local inv_archaeopteryx = {}
inv_archaeopteryx.archaeopteryx_number = tonumber(storage:get("archaeopteryx_number") or 1)
...
local function deserialize_inventory(inv, data)
    local items = data
    for i = 0, archaeopteryx_inv_size do
...
local function archaeopteryx_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:archaeopteryx_inv")
            end
        end
    ...
    if self.tamed then
		mob_core.random_loot_drop(self, 300, 300, "paleotest:sap")
    end
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.archaeopteryx_number = inv_archaeopteryx.archaeopteryx_number
    inv_archaeopteryx.archaeopteryx_number = inv_archaeopteryx.archaeopteryx_number + 1
    storage:set_int("archaeopteryx_number", inv_archaeopteryx.archaeopteryx_number)
    local inv = minetest.create_detached_inventory("paleotest:archaeopteryx_" .. self.archaeopteryx_number, {})
    inv:set_size("main", archaeopteryx_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:archaeopteryx_inv",
            "size[8,9]" ..
            "list[detached:paleotest:archaeopteryx_" .. self.archaeopteryx_number .. ";main;0,0;8,1;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:archaeopteryx_" .. self.archaeopteryx_number .. ";main]" ..
