...
local mammoth_inv_size = 7 * 8
local inv_mammoth = {}
inv_mammoth.mammoth_number = tonumber(storage:get("mammoth_number") or 1)
...
local function mammoth_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:mammoth_inv")
            end
        end
        
        minetest.remove_detached_inventory("mammoth_" .. self.mammoth_number)
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.mammoth_number = inv_mammoth.mammoth_number
    inv_mammoth.mammoth_number = inv_mammoth.mammoth_number + 1
    storage:set_int("mammoth_number", inv_mammoth.mammoth_number)
    local inv = minetest.create_detached_inventory("paleotest:mammoth_" .. self.mammoth_number, {})
    inv:set_size("main", mammoth_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:mammoth_saddle" and clicker:get_player_name() == self.owner then
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:mammoth_inv",
            "size[8,9]" ..
            "list[detached:paleotest:mammoth_" .. self.mammoth_number .. ";main;0,0;8,7;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:mammoth_" .. self.mammoth_number .. ";main]" ..
