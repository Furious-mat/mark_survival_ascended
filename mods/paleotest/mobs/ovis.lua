...
local ovis_inv_size = 1 * 8
local inv_ovis = {}
inv_ovis.ovis_number = tonumber(storage:get("ovis_number") or 1)
...
local function ovis_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:ovis_inv")
            end
        end
        
        minetest.remove_detached_inventory("ovis_" .. self.ovis_number)
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.ovis_number = inv_ovis.ovis_number
    inv_ovis.ovis_number = inv_ovis.ovis_number + 1
    storage:set_int("ovis_number", inv_ovis.ovis_number)
    local inv = minetest.create_detached_inventory("paleotest:ovis_" .. self.ovis_number, {})
    inv:set_size("main", ovis_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:ovis_inv",
            "size[8,9]" ..
            "list[detached:paleotest:ovis_" .. self.ovis_number .. ";main;0,0;8,1;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:ovis_" .. self.ovis_number .. ";main]" ..
		...
		if itemname == "shears:shears" and self.tamed
		and not self.gotten
		and not self.child then
		...
		for _, color in ipairs(palette) do
			if itemname:find("dye:") and self.tamed
			and not self.gotten
			and not self.child then
