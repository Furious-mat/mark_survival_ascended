...
local procoptodon_inv_size = 3 * 8
local inv_procoptodon = {}
inv_procoptodon.procoptodon_number = tonumber(storage:get("procoptodon_number") or 1)
...
local function deserialize_inventory(inv, data)
    local items = data
    for i = 0, procoptodon_inv_size do
...
local function give_xp_to_nearby_players(pos)
    ...
    for _, obj in ipairs(players) do
        if obj:is_player() then
            xp_redo.add_xp(obj:get_player_name(), 95)
        end
    end
end
...
local function procoptodon_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:procoptodon_inv")
            end
        end
        
        minetest.remove_detached_inventory("procoptodon_" .. self.procoptodon_number)
...
minetest.register_entity("paleotest:procoptodon", {
    ...
    targets = {},
    predators = {},
    follow = paleotest.global_mushroom,
    drops = {
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.procoptodon_number = inv_procoptodon.procoptodon_number
    inv_procoptodon.procoptodon_number = inv_procoptodon.procoptodon_number + 1
    storage:set_int("procoptodon_number", inv_procoptodon.procoptodon_number)
    local inv = minetest.create_detached_inventory("paleotest:procoptodon_" .. self.procoptodon_number, {})
    inv:set_size("main", procoptodon_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:procoptodon_saddle" and clicker:get_player_name() == self.owner then
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:procoptodon_inv",
            "size[8,9]" ..
            "list[detached:paleotest:procoptodon_" .. self.procoptodon_number .. ";main;0,0;8,3;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:procoptodon_" .. self.procoptodon_number .. ";main]" ..
...
minetest.register_craftitem("paleotest:procoptodon_dossier", {
	description = "Procoptodon Dossier",
	stack_max= 1,
	inventory_image = "paleotest_procoptodon_fg_male.png",
	groups = {dossier = 1},
