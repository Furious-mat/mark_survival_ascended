...
local phiomia_inv_size = 5 * 8
local inv_phiomia = {}
inv_phiomia.phiomia_number = tonumber(storage:get("phiomia_number") or 1)
...
local function phiomia_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:phiomia_inv")
            end
        end
        
        minetest.remove_detached_inventory("phiomia_" .. self.phiomia_number)
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.phiomia_number = inv_phiomia.phiomia_number
    inv_phiomia.phiomia_number = inv_phiomia.phiomia_number + 1
    storage:set_int("phiomia_number", inv_phiomia.phiomia_number)
    local inv = minetest.create_detached_inventory("paleotest:phiomia_" .. self.phiomia_number, {})
    inv:set_size("main", phiomia_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker, itemstack)
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:phiomia_saddle" and clicker:get_player_name() == self.owner then
            mob_core.mount(self, clicker)
        end
        if clicker:get_wielded_item():get_name() == "msa_cryopod:cryopod" then
        msa_cryopod.capture_with_cryopod(self, clicker)
        end
        if clicker:get_wielded_item():get_name() == "farming:stimberry" and clicker:get_player_name() == self.owner then
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:phiomia_inv",
            "size[8,9]" ..
            "list[detached:paleotest:phiomia_" .. self.phiomia_number .. ";main;0,0;8,5;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:phiomia_" .. self.phiomia_number .. ";main]" ..
