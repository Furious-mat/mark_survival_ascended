...
local carbonemys_inv_size = 4 * 8
local inv_carbonemys = {}
inv_carbonemys.carbonemys_number = tonumber(storage:get("carbonemys_number") or 1)
...
local function deserialize_inventory(inv, data)
    local items = data
    for i = 0, carbonemys_inv_size do
...
local function carbonemys_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:carbonemys_inv")
            end
        end
        
        minetest.remove_detached_inventory("carbonemys_" .. self.carbonemys_number)
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.carbonemys_number = inv_carbonemys.carbonemys_number
    inv_carbonemys.carbonemys_number = inv_carbonemys.carbonemys_number + 1
    storage:set_int("carbonemys_number", inv_carbonemys.carbonemys_number)
    local inv = minetest.create_detached_inventory("paleotest:carbonemys_" .. self.carbonemys_number, {})
    inv:set_size("main", carbonemys_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:carbonemys_saddle" and clicker:get_player_name() == self.owner then
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:carbonemys_inv",
            "size[8,9]" ..
            "list[detached:paleotest:carbonemys_" .. self.carbonemys_number .. ";main;0,0;8,4;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:carbonemys_" .. self.carbonemys_number .. ";main]" ..
