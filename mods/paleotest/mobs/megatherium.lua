...
local megatherium_inv_size = 5 * 8
local inv_megatherium = {}
inv_megatherium.megatherium_number = tonumber(storage:get("megatherium_number") or 1)
...
local function deserialize_inventory(inv, data)
    local items = data
    for i = 0, megatherium_inv_size do
...
local function megatherium_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:megatherium_inv")
            end
        end
        
        minetest.remove_detached_inventory("megatherium_" .. self.megatherium_number)
...
minetest.register_entity("paleotest:megatherium", {
    -- Stats
    max_hp = 740,
    ...
    visual = "mesh",
    mesh = "paleotest_megatherium.b3d",
    female_textures = {"paleotest_megatherium.png"},
    male_textures = {"paleotest_megatherium.png"},
    child_textures = {"paleotest_megatherium_child.png"},
    animation = {
    ...
    drops = {
        ...
        {name = "paleotest:pelt", chance = 1, min = 5, max = 10}
    },
    timeout = 0,
    logic = megatherium_logic,
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.megatherium_number = inv_megatherium.megatherium_number
    inv_megatherium.megatherium_number = inv_megatherium.megatherium_number + 1
    storage:set_int("megatherium_number", inv_megatherium.megatherium_number)
    local inv = minetest.create_detached_inventory("paleotest:megatherium_" .. self.megatherium_number, {})
    inv:set_size("main", megatherium_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:field_guide" then
            ...
            minetest.show_formspec(clicker:get_player_name(),
                                   "paleotest:megatherium_guide",
                                   paleotest.register_fg_entry(self, {
                female_image = "paleotest_megatherium_fg.png",
                male_image = "paleotest_megatherium_fg.png",
                diet = "Omnivore",
                temper = "Neutral"
            }))
        end
        if clicker:get_wielded_item():get_name() == "paleotest:megatherium_saddle" and clicker:get_player_name() == self.owner then
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:megatherium_inv",
            "size[8,9]" ..
            "list[detached:paleotest:megatherium_" .. self.megatherium_number .. ";main;0,0;8,5;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:megatherium_" .. self.megatherium_number .. ";main]" ..
...
mob_core.register_spawn_egg("paleotest:megatherium", "473f33cc", "393225d9")

minetest.register_craftitem("paleotest:megatherium_dossier", {
	description = "Megatherium Dossier",
	stack_max= 1,
	inventory_image = "paleotest_megatherium_fg.png",
	groups = {dossier = 1},
