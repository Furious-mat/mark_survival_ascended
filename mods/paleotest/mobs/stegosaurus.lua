...
local stegosaurus_inv_size = 4 * 8
local inv_stegosaurus = {}
inv_stegosaurus.stegosaurus_number = tonumber(storage:get("stegosaurus_number") or 1)
...
local function deserialize_inventory(inv, data)
    local items = data
    for i = 0, stegosaurus_inv_size do
...
local function stegosaurus_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:stegosaurus_inv")
            end
        end
        
        minetest.remove_detached_inventory("stegosaurus_" .. self.stegosaurus_number)
...
minetest.register_entity("paleotest:stegosaurus", {
    ...
    driver_attach_at = {x = 0, y = 0.900, z = 0},
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.stegosaurus_number = inv_stegosaurus.stegosaurus_number
    inv_stegosaurus.stegosaurus_number = inv_stegosaurus.stegosaurus_number + 1
    storage:set_int("stegosaurus_number", inv_stegosaurus.stegosaurus_number)
    local inv = minetest.create_detached_inventory("paleotest:stegosaurus_" .. self.stegosaurus_number, {})
    inv:set_size("main", stegosaurus_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
        if paleotest.feed_tame(self, clicker, 55, true, true) then
            return
        end
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:stegosaurus_saddle" and clicker:get_player_name() == self.owner then
        ...
        minetest.show_formspec(clicker:get_player_name(), "paleotest:stegosaurus_inv",
            "size[8,9]" ..
            "list[detached:paleotest:stegosaurus_" .. self.stegosaurus_number .. ";main;0,0;8,4;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:stegosaurus_" .. self.stegosaurus_number .. ";main]" ..
...
minetest.register_craftitem("paleotest:stegosaurus_dossier", {
	description = "Stegosaurus Dossier",
	stack_max= 1,
	inventory_image = "paleotest_stegosaurus_fg_male.png",
	groups = {dossier = 1},
