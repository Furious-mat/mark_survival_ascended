...
local diplocaulus_inv_size = 1 * 8
local inv_diplocaulus = {}
inv_diplocaulus.diplocaulus_number = tonumber(storage:get("diplocaulus_number") or 1)
...
local function deserialize_inventory(inv, data)
    local items = data
    for i = 0, diplocaulus_inv_size do
...
local function diplocaulus_logic(self)
    ...
    if self.hp <= 0 then
        ...
        if self.owner then
            ...
            if player then
                minetest.close_formspec(player:get_player_name(), "paleotest:diplocaulus_inv")
            end
        end
        
        minetest.remove_detached_inventory("diplocaulus_" .. self.diplocaulus_number)
...
minetest.register_entity("paleotest:diplocaulus", {
    -- Stats
    max_hp = 250,
    ...
    visual = "mesh",
    mesh = "paleotest_diplocaulus.b3d",
    textures = {"paleotest_diplocaulus.png"},
    child_texture = {"paleotest_diplocaulus_child.png"},
    animation = {
    ...
    sounds = {
        ...
        random = {
            name = "paleotest_diplocaulus_idle",
            gain = 1.0,
            distance = 16
        },
        hurt = {
            name = "paleotest_diplocaulus_hurt",
            gain = 1.0,
            distance = 16
        },
        death = {
            name = "paleotest_diplocaulus_death",
            gain = 1.0,
            distance = 16
        }
    },
...
on_activate = function(self, staticdata, dtime_s)
    ...
    self.diplocaulus_number = inv_diplocaulus.diplocaulus_number
    inv_diplocaulus.diplocaulus_number = inv_diplocaulus.diplocaulus_number + 1
    storage:set_int("diplocaulus_number", inv_diplocaulus.diplocaulus_number)
    local inv = minetest.create_detached_inventory("paleotest:diplocaulus_" .. self.diplocaulus_number, {})
    inv:set_size("main", diplocaulus_inv_size)
    self.inv = inv
    ...
    on_rightclick = function(self, clicker)
		...
		if player and self.tamed then
			player:set_breath(100)
		end
        if paleotest.feed_tame(self, clicker, 30, true, true) then
            return
        end
        if clicker:get_wielded_item():get_name() == "" and self.tamed then
        ...
        if clicker:get_wielded_item():get_name() == "paleotest:field_guide" then
            minetest.show_formspec(clicker:get_player_name(),
                                   "paleotest:diplocaulus_guide",
                                   paleotest.register_fg_entry(self, {
                female_image = "paleotest_diplocaulus_fg.png",
                male_image = "paleotest_diplocaulus_fg.png",
                diet = "Carnivore",
                temper = "Patient"
            }))
        end
        if clicker:get_wielded_item():get_name() == "" and clicker:get_player_control().sneak == false and clicker:get_player_name() == self.owner then
        minetest.show_formspec(clicker:get_player_name(), "paleotest:diplocaulus_inv",
            "size[8,9]" ..
            "list[detached:paleotest:diplocaulus_" .. self.diplocaulus_number .. ";main;0,0;8,1;]" ..
            "list[current_player;main;0,6;8,3;]" ..
            "listring[detached:paleotest:diplocaulus_" .. self.diplocaulus_number .. ";main]" ..
...
mob_core.register_spawn_egg("paleotest:diplocaulus", "444b28cc", "383d20d9")

minetest.register_craftitem("paleotest:diplocaulus_dossier", {
	description = "Diplocaulus Dossier",
	stack_max= 1,
	inventory_image = "paleotest_diplocaulus_fg.png",
	groups = {dossier = 1},
