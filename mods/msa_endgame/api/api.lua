...
function msa_endgame.random_id()
    local idst = ""
...
function msa_endgame.goto_next_waypoint(self, tpos)
    local _, pos2 = mob_core.get_next_waypoint(self, tpos)
    msa_endgame.find_path(self, tpos)
...
function msa_endgame.hq_summon(self, prty, look_at)
    local tyaw = 0
    local spawn_at = {}
    local init = false
    local spawn_timer = 4
    ...
    local func = function(self)
        ...
        for i = 1, #spawn_at do
            ...
            and spawned_entity < 20 then
                ...
                local defense_unit = minetest.add_entity(spawn_at[i], "msa_endgame:gamma_defense_unit")
                local attack_drone = minetest.add_entity(spawn_at[i], "msa_attack_drone:gamma_attack_drone")
...
function msa_endgame.hq_fire_scepter(self, prty, target)
    local tyaw = 0
    local init = false
    local timer = 1
    local anim = ""
    ...
    local func = function(self)
        ...
        if vector.distance(pos, tpos) > 12 then
            msa_endgame.goto_next_waypoint(self, tpos)
        else
            if not self.mounted
            ...
            if timer <= 0 then
                local cube = minetest.add_entity(pos, "msa_endgame:overseer_laser")
...
function msa_endgame.hq_hunt(self, prty, target)
...
function msa_endgame.gamma_overseer_attack_player(self, prty, player) -- Attack player
    player = player
    if player
    and player:get_pos()
    and vector.distance(self.object:get_pos(), player:get_pos()) < self.view_range
    and mobkit.is_alive(player) then
        msa_endgame.hq_fire_scepter(self, prty, player)
        return
    end
    return
end

function msa_endgame.gamma_overseer_attack_mob(self, prty) -- Attack specified mobs
    if self.targets then
        for i = 1, #self.targets do
            ...
            and mobkit.is_alive(target) then
                msa_endgame.hq_fire_scepter(self, prty, target)
                return
            end
        end
    end
...
function msa_endgame.logic_attack_player(self, prty, player) -- Attack player
    player = player
    if player
    and player:get_pos()
    and vector.distance(self.object:get_pos(), player:get_pos()) < self.view_range
    and mobkit.is_alive(player) then
        msa_endgame.hq_hunt(self,prty,player)
        return
    end
    return
end

function msa_endgame.logic_attack_mobs(self, prty, tbl) -- Attack specified mobs
        ...
        for i = 1, #tbl do
            ...
            and mobkit.is_alive(target) then
                ...
                or not self.tamed then
                    msa_endgame.hq_hunt(self,prty,target)
                    return
                end
            end
        end
...
function msa_endgame.beta_hq_summon(self, prty, look_at)
    local tyaw = 0
    local spawn_at = {}
    local init = false
    local spawn_timer = 4
    ...
    local func = function(self)
        ...
        for i = 1, #spawn_at do
            ...
            and spawned_entity < 40 then
                ...
                local defense_unit = minetest.add_entity(spawn_at[i], "msa_endgame:beta_defense_unit")
                local attack_drone = minetest.add_entity(spawn_at[i], "msa_attack_drone:beta_attack_drone")
                local ent = defense_unit:get_luaentity()
                local ent = attack_drone:get_luaentity()
...
function msa_endgame.hq_beta_fire_scepter(self, prty, target)
    local tyaw = 0
    local init = false
    local timer = 0.5
    local anim = ""
    ...
    local func = function(self)
        ...
        if vector.distance(pos, tpos) > 12 then
            ...
            timer = timer - self.dtime
            if timer <= 0 then
                local cube = minetest.add_entity(pos, "msa_endgame:beta_overseer_laser")
                minetest.sound_play("shooter_rocket_fire", {gain = 1})
                cube:get_luaentity().target = target
                timer = 0.5
            end
...
function msa_endgame.beta_overseer_attack_player(self, prty, player) -- Attack player
    player = player
    if player
    and player:get_pos()
    and vector.distance(self.object:get_pos(), player:get_pos()) < self.view_range
    and mobkit.is_alive(player) then
        msa_endgame.hq_beta_fire_scepter(self, prty, player)
        return
    end
    return
end

function msa_endgame.beta_overseer_attack_mob(self, prty) -- Attack specified mobs
    if self.targets then
        for i = 1, #self.targets do
            local target = mobkit.get_closest_entity(self, self.targets[i])
            if target
            ...
            and vector.distance(self.object:get_pos(), target:get_pos()) < self.view_range
            and mobkit.is_alive(target) then
                msa_endgame.hq_beta_fire_scepter(self, prty, target)
                return
            end
        end
    end
...
function msa_endgame.alpha_hq_summon(self, prty, look_at)
    local tyaw = 0
    local spawn_at = {}
    local init = false
    local spawn_timer = 4
    ...
    local func = function(self)
        ...
        for i = 1, #spawn_at do
            ...
            and spawned_entity < 60 then
                ...
                local defense_unit = minetest.add_entity(spawn_at[i], "msa_endgame:alpha_defense_unit")
                local attack_drone = minetest.add_entity(spawn_at[i], "msa_attack_drone:alpha_attack_drone")
                local ent = defense_unit:get_luaentity()
                local ent = attack_drone:get_luaentity()
...
function msa_endgame.hq_alpha_fire_scepter(self, prty, target)
    local tyaw = 0
    local init = false
    local timer = 0
    local anim = ""
    ...
    local func = function(self)
        ...
        if vector.distance(pos, tpos) > 12 then
            ...
            timer = timer - self.dtime
            if timer <= 0 then
                local cube = minetest.add_entity(pos, "msa_endgame:alpha_overseer_laser")
                minetest.sound_play("shooter_rocket_fire", {gain = 1})
                cube:get_luaentity().target = target
                timer = 0
            end
...
function msa_endgame.alpha_overseer_attack_player(self, prty, player) -- Attack player
    player = player
    if player
    and player:get_pos()
    and vector.distance(self.object:get_pos(), player:get_pos()) < self.view_range
    and mobkit.is_alive(player) then
        msa_endgame.hq_alpha_fire_scepter(self, prty, player)
        return
    end
    return
end

function msa_endgame.alpha_overseer_attack_mob(self, prty) -- Attack specified mobs
    if self.targets then
        for i = 1, #self.targets do
            local target = mobkit.get_closest_entity(self, self.targets[i])
            if target
            ...
            and vector.distance(self.object:get_pos(), target:get_pos()) < self.view_range
            and mobkit.is_alive(target) then
                msa_endgame.hq_alpha_fire_scepter(self, prty, target)
                return
            end
        end
    end
