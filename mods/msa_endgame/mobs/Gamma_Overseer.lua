...
local function gamma_overseer_logic(self)
    ...
    if self.hp <= 0 then
        for _, ent in pairs(minetest.luaentities) do
            if ent.name
            and ent.name == "msa_endgame:gamma_defense_unit"
            and ent.name == "msa_attack_drone:gamma_attack_drone"
    ...
    if mobkit.timer(self, 1) then
        ...
        if prty < 4 then
            if self.summon_cooldown > 0 then
                if player then
                    msa_endgame.gamma_overseer_attack_player(self, 4, player)
                else
                    msa_endgame.gamma_overseer_attack_mob(self, prty) 
                end
            end
        end
        ...
        if prty < 2 then
            if player then
                if self.summon_cooldown <= 0 then
                    msa_endgame.hq_summon(self, 2, player)
                end
            end
        end
...
minetest.register_entity("msa_endgame:gamma_overseer",{
    -- Stats
    max_hp = 63360,
    ...
    drops = {
        ...
        {name = "msa_implant:gamma_ascension_specimen_implant", chance = 1, min = 1, max = 1},
        {name = "msa_artifact:over", chance = 1, min = 1, max = 1},
	...
	on_activate = function(self, staticdata, dtime_s)
        ...
        if self.gamma_overseer_id == 1 then
            self.gamma_overseer_id =
                mobkit.remember(self, "gamma_overseer_id", msa_endgame.random_id())
        end
...
mob_core.register_spawn_egg("msa_endgame:gamma_overseer", "7e5acb", "3c1aa4")
