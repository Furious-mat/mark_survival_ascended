...
local function refresh_output(inv, max_mode)
	...
	inv:set_size("output", #craftable + ((6*6) - (#craftable%(6*6))))
	inv:set_list("output", craftable)
end

local function make_formspec(row, item_count, max_mode)
	if item_count < (6*6) then
		row = 0
	elseif (row*8)+(6*6) > item_count then
		row = (item_count - (6*6)) / 8
	end
	...
	local inventory = {
		...
		, "list[context;output;4.4,0;6,6;" , tostring(row*8), "]"
	...
	if item_count > ((row/6)+1) * (6*6) then
...
minetest.register_node("crafting:fabricator", {
	description = S("Fabricator"),
	drawtype = "normal",
	tiles = {"machine_top.png", "machine_black.png",
		"machine_front.png", "machine_back.png",
		"machine_side2.png", "machine_side.png"},
...
local fabricator_recipe = {
	output = "crafting:fabricator",
	recipe = {
			{"paleotest:oil_ore","quartz:quartz_crystal","quartz:quartz_crystal","paleotest:oil_ore"},
			{"paleotest:oil_ore","default:steel_ingot","default:steel_ingot","paleotest:oil_ore"},
			{"paleotest:oil_ore","paleotest:sparkpowder","paleotest:sparkpowder","paleotest:oil_ore"},
			{"paleotest:oil_ore","group:paste","group:paste","paleotest:oil_ore"},
			{"paleotest:oil_ore","quartz:quartz_crystal","quartz:quartz_crystal","paleotest:oil_ore"},
	},
}

minetest.register_craft(fabricator_recipe)
...
if minetest.get_modpath("hopper") and hopper ~= nil and hopper.add_container ~= nil then
	hopper:add_container({
		{"top", "crafting:fabricator", "store"},
		{"bottom", "crafting:fabricator", "store"},
		{"side", "crafting:fabricator", "store"},
	})
end
