...
local function beta_overseer_logic(self)
    ...
    if self.hp <= 0 then
        for _, ent in pairs(minetest.luaentities) do
            if ent.name
            and ent.name == "endgame:beta_defense_unit"
            and ent.name == "attack_drone:beta_attack_drone"
    ...
    if mobkit.timer(self, 1) then
        ...
        if prty < 4 then
            if self.beta_summon_cooldown > 0 then
                if player then
                    endgame.beta_overseer_attack_player(self, 4, player)
                else
                    endgame.beta_overseer_attack_mob(self, prty) 
                end
            end
        end
        ...
        if prty < 2 then
            if player then
                if self.beta_summon_cooldown <= 0 then
                    endgame.beta_hq_summon(self, 2, player)
                end
            end
        end
...
minetest.register_entity("endgame:beta_overseer",{
    -- Stats
    max_hp = 132480,
    ...
    drops = {
        ...
        {name = "give_initial_stuff:beta_ascension_specimen_implant", chance = 1, min = 1, max = 1}
    },
    timeout = 0,
	...
	on_activate = function(self, staticdata, dtime_s)
        ...
        if self.beta_overseer_id == 1 then
            self.beta_overseer_id =
                mobkit.remember(self, "beta_overseer_id", endgame.random_id())
        end
...
mob_core.register_spawn_egg("endgame:beta_overseer", "7e5acb", "3c1aa4")
